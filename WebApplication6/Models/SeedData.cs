...
namespace WebApplication6.Models
{
    public static class SeedData
    {
        ...
        {
            using (var scope = serviceProvider.CreateScope()) // Create a new scope
            {
                var scopedProvider = scope.ServiceProvider; // Get the scoped provider

                using var context = new ApplicationDbContext(scopedProvider.GetRequiredService<DbContextOptions<ApplicationDbContext>>());
